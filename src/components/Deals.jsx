		...
		<section className="flex flex-col px-10 lg:px-20 mx-auto ">
			...
			<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md-gap-4 lg:gap-8">
				{products?.slice(24, 32).map(item => (
					<div onClick={() => navigate(`/product_detail/${item._id}`)} key={item._id} className="fflex group  flex-col space-y-1 cursor-pointer  rounded-md bg-white h-64 md:h-72 md:w-52 lg:h-96 w-36 lg:w-64 px-2 py-6">
